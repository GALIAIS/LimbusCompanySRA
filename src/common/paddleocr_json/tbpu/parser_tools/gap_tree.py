...
class GapTree:
    def __init__(self, get_bbox: Callable[[any], Tuple[float, float, float, float]]):
        """
    ...
    # ======================= 调用接口 =====================
    # 对文本块列表排序
    def sort(self, text_blocks: List[any]) -> List[any]:
        """
        对文本块列表，按人类阅读顺序进行排序。
    ...
    # 获取以区块为单位的文本块二层列表
    def get_nodes_text_blocks(self) -> List[List[any]]:
        """
    ...
    def _get_units(self, text_blocks: List[any]) -> Tuple[
        List[Tuple[Tuple[float, float, float, float], any]], float, float]:
    ...
    def _get_cuts_rows(self, units: List[Tuple[Tuple[float, float, float, float], any]],
                       page_l: float, page_r: float) -> Tuple[
        List[Tuple[float, float, int, int]], List[List[Tuple[Tuple[float, float, float, float], any]]]]:
        # 使用间隙组 gaps2 更新 gaps1 。返回： 更新后的gaps1 , gaps1中被移除的间隙
        def update_gaps(gaps1: List[Tuple[float, float, int]],
                        gaps2: List[Tuple[float, float, int]]) -> Tuple[
            List[Tuple[float, float, int]], List[Tuple[float, float, int]]]:
            flags1 = [True] * len(gaps1)  # gaps1[i] 是否彻底移除
            flags2 = [True] * len(gaps2)  # gaps2[i] 是否新加入
            new_gaps1 = []
            for i1, (l1, r1, _) in enumerate(gaps1):
                for i2, (l2, r2, _) in enumerate(gaps2):  # 对每一个gap1，考察所有gap2
                    # 计算交集的起点和终点
                    inter_l = max(l1, l2)
                    inter_r = min(r1, r2)
                    # 如果交集有效
                    if inter_l <= inter_r:
                        # 更新 gap1 左右边缘
                        new_gaps1.append((inter_l, inter_r, gaps1[i1][2]))
                        flags1[i1] = False  # 旧的 gap1 不应移除
                        flags2[i2] = False  # 新的 gap2 不应添加
            # gap2 新加入
            new_gaps1.extend(gaps2[i2] for i2, f2 in enumerate(flags2) if f2)
            # 记录 gaps1 彻底移除的项
        ...
        while unit_index < len(units):
            ...
            for u in row:  # 遍历当前行的块
                ...
                search_start = max(search_start, r)
            # 页面右边缘 加入最后一个间隙
            ...
            completed_cuts.extend((*dg1, row_max) for dg1 in del_gaps)
            # ========== End ==========
        ...
        completed_cuts.extend((*g, row_max) for g in gaps)
    ...
    def _get_layout_tree(self, cuts: List[Tuple[float, float, int, int]],
                         rows: List[List[Tuple[Tuple[float, float, float, float], any]]]) -> dict:
        ...
        def complete(node: dict):
            ...
            for com_node in completed_nodes:
                # 父节点的垂直投影必须包含当前右界
                if not (com_node["x_left"] <= node_r <= com_node["x_right"] + 0.0001):
                    continue
                # 父节点底部必须在当前之上
    ...
    # ======================= 从节点序列中，提取原始文本块序列 =====================
    def _get_text_blocks(self, nodes: List[dict]) -> List[any]:
        result = []
        for node in nodes:
            result.extend(unit[1] for unit in node["units"])
        return result
