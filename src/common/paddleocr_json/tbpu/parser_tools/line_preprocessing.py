...
# 计算一个box的旋转角度
def _calculateAngle(box: List[Tuple[float, float]]) -> float:
    # 获取宽高
...
# 估计一组文本块的旋转角度
def _estimateRotation(textBlocks: List[Dict]) -> float:
...
def _getBboxes(textBlocks: List[Dict], rotation_rad: float) -> List[Tuple[float, float, float, float]]:
...
def linePreprocessing(textBlocks: List[Dict]) -> List[Dict]:
    # 判断角度
