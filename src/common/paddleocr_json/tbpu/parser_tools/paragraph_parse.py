...
def word_separator(letter1: str, letter2: str) -> str:
    ...
    def is_cjk(character: str) -> bool:
        cjk_unicode_ranges: List[Tuple[int, int]] = [
            (0x4E00, 0x9FFF),  # 中文
            (0x3040, 0x30FF),  # 日文
            (0x1100, 0x11FF),  # 韩文
...
class ParagraphParse:
    def __init__(self, get_info: Callable[[Any], Tuple[Tuple[float, float, float, float], str]],
                 set_end: Callable[[Any, str], None]) -> None:
        """
    ...
    def run(self, text_blocks: List[Any]) -> List[Any]:
        """
        ...
        """
        # 封装块单元
        units: List[Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]] = self._get_units(text_blocks)
        # 执行分析
        self._parse(units)
        return text_blocks
    ...
    def _get_units(self, text_blocks: List[Any]) -> List[
        Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]]:
        return [(self.get_info(tb)[0], (self.get_info(tb)[1][0], self.get_info(tb)[1][-1]), tb) for tb in text_blocks]

    # ======================= 分析 =====================

    # 执行分析
    def _parse(self, units: List[Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]]) -> List[
        Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]]:
        units.sort(key=lambda a: a[0][1])  # 确保从上到下有序
        para_l, para_top, para_r, para_bottom = units[0][0]  # 当前段的左右
        para_line_h: float = para_bottom - para_top  # 当前段行高
        para_line_s: float = None  # 当前段行间距
        now_para: List[Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]] = [units[0]]  # 当前段的块
        paras: List[List[Tuple[Tuple[float, float, float, float], Tuple[str, str], Any]]] = []  # 总的段
        paras_line_space: List[float] = []  # 总的段的行间距
        # 取 左右相等为一个自然段的主体
        for i in range(1, len(units)):
            l, top, r, bottom = units[i][0]  # 当前块上下左右边缘
            h: float = bottom - top
            ls: float = top - para_bottom  # 行间距
            # 检测是否同一段
            if (  # 左右边缘都相等
        ...
        for i1 in reversed(range(len(paras))):
            ...
            if len(para) == 1:
                ...
                if i1 > 0:
                    ...
                    # 检查行间距
                    if paras_line_space[i1 - 1] is not None and top - up_bottom > paras_line_space[i1 - 1] + up_h * 0.5:
                        up_flag = False
                # 下段开头条件：右对齐/单行超出，左缩进
                if i1 < len(paras) - 1:
                    ...
                    # 检查行间距
                    if paras_line_space[i1 + 1] is not None and down_top - bottom > paras_line_space[
                        i1 + 1] + down_h * 0.5:
                        down_flag = False
