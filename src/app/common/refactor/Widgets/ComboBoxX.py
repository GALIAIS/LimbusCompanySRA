...
class ComboBoxBaseX:
    ...
    def addItem(self, text, icon: Union[str, QIcon, FluentIconBase] = None, userData=None):
        ...
        icon: str | QIcon | FluentIconBase
        """
        item = ComboItemX(text, icon, userData)
...
class ComboBoxX(QPushButton, ComboBoxBaseX):
    """ Combo box """
    ...
    def __init__(self, parent=None, config_key: str = None):  # 添加 config_key 参数
        super().__init__(parent=parent)
        self.config_key = config_key  # 存储 config_key
        ...
        # 初始化时从配置中读取值
        if self.config_key:
            initial_index = self.findData(cfgm.get(self.config_key))
            if initial_index >= 0:
    ...
    def setCurrentIndex(self, index: int):
        if index < 0:
            ...
            # 更新配置值
            if self.config_key:
                cfgm.set(self.config_key, self.currentData())
...
class EditableComboBoxX(LineEdit, ComboBoxBaseX):
    """ Editable combo box """
    ...
    def __init__(self, parent=None, config_key: str = None):  # 添加 config_key 参数
        ...
        # 初始化时从配置中读取值
        if self.config_key:
            initial_text = cfgm.get(self.config_key)
            if initial_text:
    ...
    def setCurrentIndex(self, index: int):
        ...
        if index < 0:
            ...
            # 更新配置值
            if self.config_key:
                cfgm.set(self.config_key, self.currentText())

    def clear(self):
        ComboBoxBaseX.clear(self)
    ...
    def _onClearButtonClicked(self):
        ...
        # 清空配置值
        if self.config_key:
            cfgm.set(self.config_key, "")


class ComboBoxMenuX(RoundMenu):
    """ Combo box menu """
